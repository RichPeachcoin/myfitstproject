document.addEventListener("DOMContentLoaded", function() {
    ...
    function resetStyles() {
        var topLine = document.querySelector(".menu-icon1_line-top");
        var middleLine = document.querySelector(".menu-icon1_line-middle");
        var bottomLine = document.querySelector(".menu-icon1_line-bottom");
        topLine.classList.remove("rotate45");
        bottomLine.classList.remove("rotateMinus45");
    ...
    menuButton.addEventListener("click", function() {
        ...
        if (topLine.classList.contains("rotate45")) {
            // Если уже повернуты, сбросить все в исходное состояние
            resetStyles();
        } else {
            // Если не повернуты, повернуть и скрыть среднюю линию
            topLine.classList.add("rotate45");
            bottomLine.classList.add("rotateMinus45");
            middleLine.style.opacity = "0";
        }
    });

    // Обработчики событий для элементов навигации
    var navLinks = document.querySelectorAll(".navbar1_link");
    navLinks.forEach(function(link) {
        link.addEventListener("click", resetStyles);
    });
});

// additional
document.addEventListener("DOMContentLoaded", function() {
  const links = document.querySelectorAll('.navbar1_link');
  ...
  links.forEach(link => {
    link.addEventListener('click', function(event) {
      ...
      if (targetElement) {
        ...
        // Если секция имеет класс, учитываем его
        if (targetElement.classList.contains('section id="about"') || 
            targetElement.classList.contains('section id="tokenomics"') || 
            targetElement.classList.contains('section id="roadmap"')) {
          offset += targetElement.getBoundingClientRect().height;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    const burgerButton = document.querySelector('.navbar1_menu-button');
    const menu = document.querySelector('.navbar1_menu');

    burgerButton.addEventListener('click', function() {
        menu.classList.toggle('is-open');
        burgerButton.setAttribute('aria-expanded', menu.classList.contains('is-open'));
    });
});
...
function copyToClipboard(event) {
    ...
    const textToCopy = 'coming soon, stay tuned';
    ...
    // Возвращение текста кнопки обратно на "copy" через 2 секунды
    setTimeout(() => {
...
(() => {
  ...
  if (!data.done.attr("aria-label")) {
    data.done.attr("aria-label", formName);
  }
